          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            setMovimentacoes(
              ordenado.map((item: any) => ({
                ...
                valor: item.valor,
                origem:
                  item.origem || (item.valor > 0 ? "Depósito" : "Despesa"),
  ...
  const movimentacoesFiltradas =
    filtrar && filtroData
      ? movimentacoes.filter(
          (item) => item.data === filtroData.toISOString().slice(0, 10)
        )
      : movimentacoes;
      ...
      <View style={styles.filtroRow}>
        ...
        >
          ...
          <Text style={styles.filtrarBtnText}>
            {filtroData
              ? filtroData.toLocaleDateString("pt-BR")
              : "Selecionar data"}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            styles.filtrarBtn,
            { backgroundColor: "#228B22", marginLeft: 8 },
          ]}
          onPress={() => setFiltrar(true)}
          disabled={!filtroData}
        >
          <MaterialIcons name="search" size={20} color="#fff" />
          <Text style={[styles.filtrarBtnText, { color: "#fff" }]}>
            Filtrar
          </Text>
        </TouchableOpacity>
        {filtrar && (
